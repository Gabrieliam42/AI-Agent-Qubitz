...
ROUTE_PROFILE_GLM_24G = "24g_retrieval"
...
def _build_local_mcp_server(base: Any, workspace: Path, runtime_workspace: Path, launch_script: Path) -> Any:
    ...
    @server.tool(
        description=(
            "Start a persistent local server process as a managed background job using a direct project Python "
            "interpreter path. Do not use this for a session-scoped stdio MCP script passed to "
            "list_project_mcp_tools or call_project_mcp_tool."
        )
    )
    ...
    @server.tool(
        description=(
            "List tools from a local MCP server. An explicit script or configuration server_reference is opened as "
            "a session-scoped stdio process and closed automatically; it does not require start_project_mcp_server. "
    ...
    @server.tool(
        description=(
            "Invoke one named local MCP tool. An explicit script or configuration server_reference is opened as a "
            "session-scoped stdio process and closed automatically; it does not require start_project_mcp_server. "
            "A persistent managed server_id is also accepted."
        )
    )
