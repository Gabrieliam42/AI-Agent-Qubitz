...
QWEN_12G_RTX3090_NUM_CTX = 101376
...
def _clamp_qwen_12g_num_predict(value: Any) -> int:
    try:
        ...
        numeric = QWEN_12G_DEFAULT_NUM_PREDICT
    return max(1, min(numeric, QWEN_12G_MAX_NUM_PREDICT))
...
def _apply_qwen_12g_profile(base: Any) -> None:
    ...
    if getattr(base.LlamaCppServerProcess, "_qubitz_qwen_12g_patched", False):
        return
    original_launch_command = base.LlamaCppServerProcess._launch_command

    def _launch_command_qwen_12g(server_self: Any) -> list[str]:
        if hasattr(server_self.config, "num_ctx"):
            server_self.config.num_ctx = _clamp_qwen_12g_num_ctx(getattr(server_self.config, "num_ctx"))
    ...
    base.LlamaCppServerProcess._launch_command = _launch_command_qwen_12g
    base.LlamaCppServerProcess._qubitz_qwen_12g_patched = True
...
_EMBEDDED_BASE_SOURCE = _EMBEDDED_BASE_SOURCE.replace(
    "Qwen3.5-9B-Q8_0.gguf", "Qwen3.5-9B-UD-Q5_K_XL.gguf"
)
_EMBEDDED_BASE_SOURCE = _EMBEDDED_BASE_SOURCE.replace(
    "Qwen3.5-9B-Q8", "Qwen3.5-9B-UD-Q5_K_XL"
)
_EMBEDDED_BASE_SOURCE = _EMBEDDED_BASE_SOURCE.replace(
...
class LocalOnlyApp:
    def __init__(self, base_module_name: str, wrapper_script: str, display_name: str) -> None:
        ...
        _apply_qwen_12g_profile(self.base)
        ...
        class EnhancedGUI(base.QubitzGUI):
            ...
            def __init__(self, config: Any) -> None:
                ...
                self._append_transcript(
                    "system",
                    (
                        "12 GB Qwen3.5 UD-Q5_K_XL profile active. Context is capped at 101376 on every GPU, Max Output is capped at 8096, "
            ...
            def _sync_runtime_settings(self) -> None:
                ...
                clamped_num_ctx = _clamp_qwen_12g_num_ctx(self.config.num_ctx)
                clamped_num_predict = _clamp_qwen_12g_num_predict(self.config.num_predict)
    ...
    def _build_config(self, args: Any, workspace: Path) -> Any:
        ...
        if hasattr(args, "num_predict"):
            if "ollama_num_predict" in getattr(self.base.AgentConfig, "__dataclass_fields__", {}):
                ...
                kwargs["num_predict"] = _clamp_qwen_12g_num_predict(args.num_predict)
        ...
        if hasattr(config, "num_ctx"):
            config.num_ctx = _clamp_qwen_12g_num_ctx(getattr(config, "num_ctx"))
        if hasattr(config, "num_predict"):
            config.num_predict = _clamp_qwen_12g_num_predict(getattr(config, "num_predict"))
        if hasattr(config, "embed_device"):
            config.embed_device = QWEN_12G_EMBED_DEVICE
