...
class LocalMCPServerManager:
    ...
    def _resolve_reference(self, workspace: Path, server_reference: str) -> str:
        reference = server_reference.strip()
        if "://" in reference:
            return reference
        return str(self._resolve_workspace_path(workspace, reference, allow_missing=False))
...
def _find_available_local_port(host: str = "127.0.0.1") -> int:
...
def _patch_local_only_dependencies(base: Any) -> None:
    ...
    def _can_rebind_to_fresh_local_port(self: Any) -> bool:
        return (
            self._can_restart_mismatched_server()
            and callable(getattr(base, "in_wsl", None))
            and base.in_wsl()
            and not _wsl_windows_executable_interop_available(base)
        )
    ...
    def _restart_on_fresh_local_port(self: Any, timeout_seconds: float) -> None:
        if self.server_process is not None:
            try:
                ...
                pass
            self.server_process.process = None
        port = _find_available_local_port(getattr(base, "DEFAULT_LLAMACPP_HOST", "127.0.0.1"))
        fresh_url = base.normalize_base_url(
            f"http://{getattr(base, 'DEFAULT_LLAMACPP_HOST', '127.0.0.1')}:{port}"
        )
        self.config.server_url = fresh_url
        os.environ[base.LLAMACPP_SERVER_URL_ENV_NAME] = fresh_url
        self.server_process = base.LlamaCppServerProcess(self.config)
        self.server_process.ensure_started()
        self.transports = []
        self.transport = base.DirectHTTPTransport(fresh_url, "direct")
        deadline = time.time() + min(timeout_seconds, 120.0)
        while time.time() < deadline:
            transports = self._reachable_transports(fresh_url)
            if transports:
                self.transports = list(transports)
                self.transport = self.transports[0]
                return
            ...
            if process is not None and process.poll() is not None:
                ...
                log_hint = f"\nServer log tail:\n{log_tail}" if log_tail else ""
                raise RuntimeError(
                    f"llama-server exited while rebinding the requested model to {fresh_url}.{log_hint}"
                )
            time.sleep(0.5)
        log_tail = self.server_process.read_log_tail()
        log_hint = f"\nServer log tail:\n{log_tail}" if log_tail else ""
        raise RuntimeError(
            f"Restarted llama-server on {fresh_url}, but the server did not become reachable in time.{log_hint}"
        )
    ...
    def ensure_model(
        ...
        if available_ids and self._can_restart_mismatched_server():
            ...
            if model_name in available_ids:
                return model_name
            if available_ids and self._can_rebind_to_fresh_local_port():
                self.last_model_resolution_note = (
                    f"Port {getattr(base, 'DEFAULT_LLAMACPP_PORT', 8001)} is still occupied by mismatched model "
                    f"({', '.join(available_ids)}); rebinding the requested GGUF to a fresh local port."
                )
                self._restart_on_fresh_local_port(timeout_seconds)
                models, props_available, saw_loading, last_model_probe, last_props_probe = self._wait_for_model_state(
                    timeout_seconds
                )
                if props_available and not models:
    ...
    base.LlamaCppClient._can_rebind_to_fresh_local_port = _can_rebind_to_fresh_local_port
    base.LlamaCppClient._restart_with_requested_model = _restart_with_requested_model
    base.LlamaCppClient._restart_on_fresh_local_port = _restart_on_fresh_local_port
...
class LocalOnlyApp:
    ...
    def _build_mcp_host_class(self) -> type[Any]:
        ...
        class EnhancedMCPHost(base.MCPHost):
            def _server_parameters(self_nonlocal: Any) -> Any:
                ...
                server_url = env.get(getattr(base, "LLAMACPP_SERVER_URL_ENV_NAME", ""), "").strip()
                if server_url:
                    args.extend(["--server-url", server_url])
                model_path = env.get(getattr(base, "GGUF_MODEL_PATH_ENV_NAME", ""), "").strip()
                if model_path:
                    args.extend(["--model-path", model_path])
                llama_server = env.get(getattr(base, "LLAMACPP_SERVER_PATH_ENV_NAME", ""), "").strip()
                if llama_server:
                    args.extend(["--llama-server", llama_server])
    ...
    def _build_agent_runner_class(self) -> type[Any]:
        ...
        class EnhancedAgentRunner(base.AgentRunner):
            def _is_simple_direct_question(self, prompt: str) -> bool:
            ...
            def _adaptive_step_budget(self, prompt: str) -> tuple[int, list[str]]:
                ...
                setattr(self.config, "max_steps", self._effective_max_steps())
                try:
            ...
            async def _run_async(self, prompt: str, callback: Callable[[str, str], None] | None = None) -> str:
                ...
                server_url_env_name = getattr(base, "LLAMACPP_SERVER_URL_ENV_NAME", "")
                model_path_env_name = getattr(base, "GGUF_MODEL_PATH_ENV_NAME", "")
                llama_server_env_name = getattr(base, "LLAMACPP_SERVER_PATH_ENV_NAME", "")
                original_env_values = {
                    server_url_env_name: os.environ.get(server_url_env_name) if server_url_env_name else None,
                    model_path_env_name: os.environ.get(model_path_env_name) if model_path_env_name else None,
                    llama_server_env_name: os.environ.get(llama_server_env_name) if llama_server_env_name else None,
                }
                ...
                if server_url_env_name and getattr(self.config, "server_url", ""):
                    os.environ[server_url_env_name] = str(getattr(self.config, "server_url"))
                if model_path_env_name and getattr(self.config, "model_path", ""):
                    os.environ[model_path_env_name] = str(getattr(self.config, "model_path"))
                if llama_server_env_name and getattr(self.config, "llama_server_path", ""):
                    os.environ[llama_server_env_name] = str(getattr(self.config, "llama_server_path"))
                simple_direct_question = self._is_simple_direct_question(prompt)
                original_cache = self._tool_definitions_cache
                original_count = self._tool_count_cache
                if simple_direct_question:
                    self._emit(
                        callback,
                        "status",
                        "Simple direct question detected; skipping MCP tool loading so the model can answer directly.",
                    )
                ...
                try:
                    ...
                    for env_name, previous_value in original_env_values.items():
                        if not env_name:
                            continue
...
_APP = build_local_only_app(
    'AI_Agent_Qubitz_All_Local',
    __file__,
    'AI Agent Qubitz All Local Local-Only',
)
