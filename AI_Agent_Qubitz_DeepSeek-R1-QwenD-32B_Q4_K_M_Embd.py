    ...
    def _build_agent_runner_class(self) -> type[Any]:
        ...
        class EnhancedAgentRunner(base.AgentRunner):
            ...
            def _run_existing_script_preflight(
                ...
                target_count = expected_count or len(structured_rows)
                if (
                    return_code == 0
                    and target_count > 0
                    and len(structured_rows) >= target_count
                    and (not requires_url or all(str(row.get("url", "")).strip() for row in structured_rows[:target_count]))
                    and (not requested_fields or _rows_have_required_fields(structured_rows[:target_count], requested_fields))
                ):
                    self._existing_script_direct_answer = _format_direct_result_answer(
                        structured_rows[:target_count],
            ...
            async def _run_async(self, prompt: str, callback: Callable[[str, str], None] | None = None) -> str:
                ...
                if bypass_retrieval:
                    ...
                    if self._existing_script_direct_answer:
                        return self._existing_script_direct_answer
