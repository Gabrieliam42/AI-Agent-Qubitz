...
class LocalOnlyApp:
    ...
    def _build_agent_runner_class(self) -> type[Any]:
        ...
        class EnhancedAgentRunner(base.AgentRunner):
            ...
            def _select_task_guidance(self, prompt: str) -> str:
                ...
                if selected_route == "simple_answer":
                    return (
                        ...
                        "- Answer directly in one short sentence when possible.\n"
                        "- Do not restate the question or explain your reasoning.\n"
            ...
            def __init__(self, config: Any) -> None:
                ...
                self._ump_context = ""
                self._background_model_lock = threading.RLock()
                self._background_model_prewarm_in_progress = False
            ...
            def _get_llm(self) -> Any:
                with self._background_model_lock:
                    return super()._get_llm()

            def reset_runtime(self) -> None:
                with self._background_model_lock:
                    return super().reset_runtime()

            def _prepare_local_runtime_for_llm(
                self,
                callback: Callable[[str, str], None] | None = None,
                *,
                ...
                configured_server_url = (os.environ.get(base.LLAMACPP_SERVER_URL_ENV_NAME) or "").strip()
                if in_wsl_session and interop_available and emit_status:
                    self._emit(
                        callback,
                        "status",
            ...
            def prewarm_local_model(self) -> str | None:
                with self._background_model_lock:
                    if (
                        self._background_model_prewarm_in_progress
                        or self._background_model_prewarm_completed
                        or self._background_model_prewarm_failed
                    ):
                        return None
                    self._background_model_prewarm_in_progress = True
                    self._background_model_prewarm_error = ""
                model_label = getattr(base, "DEFAULT_GGUF_MODEL_LABEL", self.config.model_name)
                try:
                    self._prepare_local_runtime_for_llm(emit_status=False)
                    llm = self._get_llm()
                    llm.ensure_model(
                        self.config.model_name,
                        timeout_seconds=float(getattr(base, "DEFAULT_LLAMACPP_MODEL_READY_TIMEOUT_SECONDS", 300.0)),
                    )
                    resolution_note = (getattr(llm, "last_model_resolution_note", "") or "").strip()
                except Exception as exc:
                    with self._background_model_lock:
                        self._background_model_prewarm_in_progress = False
                        self._background_model_prewarm_failed = True
                        self._background_model_prewarm_error = str(exc)
                    return f"Background model prewarm failed for {model_label}: {exc}"
                with self._background_model_lock:
                    self._background_model_prewarm_in_progress = False
                    self._background_model_prewarm_completed = True
                    self._background_model_prewarm_failed = False
                    self._background_model_prewarm_error = ""
                if resolution_note:
            ...
            async def _run_async(self, prompt: str, callback: Callable[[str, str], None] | None = None) -> str:
                ...
                if selected_route == "simple_answer":
                    ...
                    self._emit(
                        callback,
                        "status",
                        "DeepSeek concise route: reasoning budget disabled for this simple direct question.",
                    )
                elif selected_route == "read_only_workspace":
                    base.set_qubitz_reasoning_budget(0)
                    base.set_qubitz_reasoning_mode("off")
                    self._emit(
                        callback,
                        "status",
                        "DeepSeek grounded read-only route: reasoning disabled; complex routes remain unrestricted.",
                    )
                if selected_route == "ask_user_missing_info":
                    self._emit(
                        callback,
                ...
                effective_thinking_effort = (
                    SIMPLE_DIRECT_QUESTION_FORCED_THINKING_EFFORT
                    if bypass_retrieval
                    else getattr(self.config, "thinking_effort", "default")
                )
                ...
                self._emit(
                    callback,
                    "status",
                    f"Thinking effort preset: {_display_thinking_effort(effective_thinking_effort)}.",
                )
                ...
                original_num_predict = getattr(self.config, "num_predict", None)
                original_thinking_effort = getattr(self.config, "thinking_effort", "default")
                if bypass_retrieval:
                    ...
                    capped_num_predict = original_num_predict
                    with suppress(Exception):
                        configured_num_predict = int(getattr(self.config, "num_predict", 0))
                        if configured_num_predict > 0:
                            capped_num_predict = min(
                                configured_num_predict,
                                SIMPLE_DIRECT_QUESTION_NUM_PREDICT_CAP,
                            )
                            self.config.num_predict = capped_num_predict
                    setattr(self.config, "thinking_effort", SIMPLE_DIRECT_QUESTION_FORCED_THINKING_EFFORT)
                    self._emit(
                        callback,
                        "status",
                        (
                            "Simple direct answer profile active: forcing low effort and capping max output to "
                            f"{getattr(self.config, 'num_predict', capped_num_predict)} tokens."
                        ),
                    )
                    ...
                    self._existing_script_preflight_context = self._run_existing_script_preflight(user_prompt, callback)
                ...
                try:
                    ...
                    if original_num_predict is not None:
                        self.config.num_predict = original_num_predict
                    setattr(self.config, "thinking_effort", original_thinking_effort)
            ...
            def _system_prompt(
                ...
                if self._simple_direct_mode:
                    ...
                    prompt = textwrap.dedent(
                        ...
                        If one sentence is insufficient, use at most two short sentences.
    ...
    def _build_gui_class(self) -> type[Any]:
        ...
        class EnhancedGUI(base.QubitzGUI):
            ...
            def _change_workspace(self) -> None:
                previous_agent = getattr(self, "agent", None)
                previous_workspace = getattr(self.config, "workspace", None)
                super()._change_workspace()
                if hasattr(self, "thinking_effort_var"):
                    effort = _normalize_thinking_effort(self.thinking_effort_var.get())
                    setattr(self.config, "thinking_effort", effort)
                    setattr(self.agent.config, "thinking_effort", effort)
                if self.agent is not previous_agent or getattr(self.config, "workspace", None) != previous_workspace:
                    self._background_model_prewarm_started = False
                    self._background_model_prewarm_cancelled = False
                    self.root.after(1200, self._maybe_start_background_model_prewarm)

            def _maybe_start_background_model_prewarm(self) -> None:
                if (
                    getattr(self, "_background_model_prewarm_started", False)
                    or getattr(self, "_background_model_prewarm_cancelled", False)
                ):
                    return
                ...
                try:
                    ...
                    return
                self._background_model_prewarm_started = True
                threading.Thread(target=self._background_model_prewarm_worker, daemon=True).start()

            def _background_model_prewarm_worker(self) -> None:
                try:
                    message = self.agent.prewarm_local_model()
                except Exception as exc:
                    message = f"Background model prewarm failed: {exc}"
                if message:
            ...
            def send_prompt(self) -> None:
                ...
                if raw_prompt:
                    ...
                    self._background_model_prewarm_cancelled = True
...
_APP = build_local_only_app(
    'AI_Agent_Qubitz_DeepSeek_R1_Distill_Qwen_32B_Q4_K_M_Embd_GeneralFast_Base',
    __file__,
    'AI Agent Qubitz DeepSeek R1 Distill Qwen 32B Q4 K M Embd General Fast Local-Only',
)
